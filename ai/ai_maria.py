...
- Foca apenas nas mensagens de Maria para personalidade, valores e gatilhos.
- Usa a data '2025-04-12' para reflexões.
Retorna SOMENTE um JSON com:
- personalidade: str (ex.: "empática", "Indefinida" se não claro)
- valores: lista de str (ex.: ["compreensão", "diálogo"])
- gatilhos_emocionais: lista de str (ex.: ["rejeição", "ignorada"])
...
"""
            try:
                ...
                print(f"📜 Resposta parseada para Maria, lote {i+1}: {profile_text[:200]}...")
                try:
    ...
    def analyze(self, blocks: list) -> dict:
        ...
        for i, batch in enumerate(batches):
            ...
            print(f"📋 Texto analisado para Maria, lote {i+1}:\n{conversation_text}\n")
            prompt = f"""
Tu és a Maria. Analisa esta conversa (lote {i+1}/{len(batches)}).
- 'Eu' refere-se a Maria; 'Rui' é a outra pessoa.
- Fala na primeira pessoa apenas para mensagens de Maria.
